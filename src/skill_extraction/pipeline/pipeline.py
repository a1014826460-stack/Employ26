...
用法::
    ...
    # 从 PostgreSQL 读取数据运行
    python -m src.skill_extraction.pipeline.pipeline run

    # 从内存数据运行（测试或自定义数据源）
    python -m src.skill_extraction.pipeline.pipeline process --help
"""
...
from .source import fetch_latest_job_description_records
...
class V3Pipeline:
    ...
    def process_record(self, record: Dict[str, Any]) -> RecordResult:
        ...
        if self._llm_client is not None and raw_soft:
            from ..soft.llm_validator import validate_soft_skills
    ...
    def run(
        ...
        records = self._fetch_records(source_table)
    ...
    @staticmethod
    def _fetch_records(source_table: str | None = None) -> List[Dict[str, Any]]:
        """读取 PostgreSQL 源记录。

        实际查询逻辑放在 `source.py`，这里保留一个窄方法方便测试替换。
        """
        return fetch_latest_job_description_records(source_table)
...
用法示例:
  ...
  # 指定源表
  python -m src.skill_extraction.pipeline.pipeline run --source-table public.job_description_parsed

  # 启用 LLM 软技能验证
  python -m src.skill_extraction.pipeline.pipeline run --use-llm

  # 输出结果到 JSON 文件
  python -m src.skill_extraction.pipeline.pipeline run --output results.json
""",
    )
