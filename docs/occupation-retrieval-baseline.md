# occupation_retrieval Retrieval Baseline

当前冻结的 occupation_retrieval 检索基线不是单个模型目录，而是一套可重复执行的实验配方。后续任何 `bge-m3` 或其他方案都应先在这套固定配方上做同口径挑战，再讨论是否替换基线。
...
- 这是当前 `src.occupation_retrieval.common.load_annotations_from_pg()` 和 `src.occupation_retrieval.train_rag_round2` 的实际输入契约
...
- 模型目录：`output/occupation_retrieval/rag_round2_training/bge-large-round2-finetuned`
- 局部评估结果：`output/occupation_retrieval/rag_round2_training/evaluation_results.json`
